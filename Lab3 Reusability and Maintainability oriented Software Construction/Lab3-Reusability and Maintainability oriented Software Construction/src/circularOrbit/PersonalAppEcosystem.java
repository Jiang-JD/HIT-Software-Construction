...
public class PersonalAppEcosystem extends ConcreteCircularOrbit<Person, App> {
	...
	private final Map<String, AppInstallTime> installing = new HashMap<String, AppInstallTime>();
	 ...
	 * 	  所有域是private final的，所有Observer返回类型均是 immutable的。客户端无法拿到内部引用。
	 */
	...
	public boolean addInstalling(String appName, AppInstallTime at) {
		assert appName != null : "AppEcosystem appName is null pointer";
		assert at != null : "AppUsageTime is null pointer";
	...
	public AppInstallTime getInstalling(String name) {
		assert name != null : "AppEcosystem App name is null pointer";
...
//	protected PersonalAppEcosystem(Builder b) {
//		super(b);
//	}
	...
	public boolean addApp(App app, int index, AppInstallTime at) {
		assert at != null : "App "+app.getName()+"'s usagetime is null pointer";
	...
	public boolean addObject(App app, int index) {
		assert app != null : "App is null pointer";
		if(!contains(app) && super.addObject(app, index)) {
			return true;
		}
		return false;
