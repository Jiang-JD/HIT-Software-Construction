 ...
 *
 */
public class TrackGame extends ConcreteCircularOrbit<CentralPoint, Athlete> {
	 ...
	 * 	racetype为String类型，其他域为private final。grouping方法返回的分组做了防御性拷贝，客户端拿不到内部引用。
	 */
	
	private void checkRep() {
//		assert getTrackNum() <= 10 && getTrackNum() >= 4: "The Number of tracks is out of 4-10";
		for(int i = 0; i< getTrackNum() ; i++) {
			if(getObjects(i) != null) {
				assert getObjects(i).size() <= 4: "More than one objects on the Race Track";
			}
		}
...
//	 * @param b
//	 */
//	private TrackGame(ConcreteCircularOrbit.Builder b) {
//		super(b);
//		checkRep();
//	}
	...
	public Athlete getAthlete(int number) {
		assert number >= 0 : "Athlete number < 0";
	...
	public boolean remove(int index) {
		if(getTrackNum() -1 < 4) {
			try {
				throw new Exception("RaceTrack Number is less than 4");
			} catch (Exception e) {
				e.printStackTrace();
			}
			return false;
