...
public class AppInstallTime {
	...
	private void checkRep() {
		for(Timespan t1 : installing) {
			...
			for(Timespan t2 : installing) {
				if(!t1.equals(t2)) assert !t1.overlap(t2) : t1 + " is overlap " + t2;
			}
		}
	}
	...
	public boolean overlap(Timespan ts) {
		...
		for(Timespan t : installing) {
			if(t.overlap(ts)) return true;  //如果有时间段重合，则说明ts时间段内App处于安装状态
		}
		return false;
	}
	 ...
	 * 
	 * @return  最早起始时间点和最晚终止时间点形成的时间段,如果App没有安装，
	 * 则返回null
	 */
	public Timespan entireSpan() {
		if(installing.isEmpty()) return null;
	
		Instant start = Instant.MAX;
		Instant end = Instant.MIN;
		
		for(Timespan t : installing) {
			if(t.getStart().isBefore(start)) start = t.getStart();
			if(t.getEnd().isAfter(end)) end = t.getEnd();
		}
