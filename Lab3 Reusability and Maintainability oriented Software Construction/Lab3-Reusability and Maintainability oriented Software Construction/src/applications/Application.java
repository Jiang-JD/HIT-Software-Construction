		...
		while (f) {
			...
			case "2": {
				...
				if (m.find()) {
					...
					tg.addObject(a, Integer.valueOf(m.group(2)));
					show(Menu.SUCCESS);
				} else {
					show(Menu.ERRORINPUT);
				}
			...
			case "4": {
				...
				if (m.find()) {
					Athlete a = tg.getAthlete(Integer.valueOf(m.group(1)));
					if (a != null) {
						if (tg.remove(a, Integer.valueOf(m.group(2)))) {
							show(Menu.SUCCESS);
							break;
						}
					}
		...
		while (f) {
			...
			case "1": {
				...
				if (m.find()) {
					...
					if(Integer.valueOf(m.group(2)) > 4) {
						showl("指定轨道应为0-4");
						break;
					}
					App a = (App) af.create(des);
					eco.addObject(a, Integer.valueOf(m.group(2)));
					show(Menu.SUCCESS);
				} else {
					show(Menu.ERRORINPUT);
				}
