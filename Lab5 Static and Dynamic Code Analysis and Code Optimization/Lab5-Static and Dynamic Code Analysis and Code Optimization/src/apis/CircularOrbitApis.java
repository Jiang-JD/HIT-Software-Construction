...
public class CircularOrbitApis<E> {
  ...
  /**
   * . 计算多轨道系统中各轨道上物体分布的熵值。通俗的说，如果所有物体都 分布在同一条轨道上，其 熵值最低；如果所有物体均匀的分布在每一条轨道
   * 上，整个系统的熵值最高。 <br>
   * 轨道系统的熵值计算公式为： <br>
   * 定义轨道数量为 {@code n}，定义物体数量为 {@code m} <br>
   * 物体出现在某条轨道的概率为 {@code p}，{@code p = 1/n} <br>
   * 轨道出现k个物体的概率为 {@code P(k) = C(x,n)[p^k * (1-p)^n-k]} <br>
   * 则单条轨道k个物体的信息为 {@code l(k) = -log2(P(k))} <br>
   * 则轨道系统的信息熵为 {@code H(x) = -sum(i from 0 to n)[P(i) * l(i)]}
   * 
   * 
   * @param c 待计算轨道系统，不为空
  ...
  public double getObjectDistributionEntropy(CircularOrbit c) {
    ...
    int tracknum = c.getTrackNum();
    double probability = 1 / (double) tracknum; // Probability
    int objnum = c.getObjectNum();
    double[] lx = new double[objnum + 1];
    double[] ex = new double[tracknum];
    double hx = 0;
    // Initialize lx
    for (int i = 0; i <= objnum; i++) {
      double l = lx(i, objnum, probability);
      if (Double.isNaN(l)) {
        lx[i] = 0;
      } else {
        lx[i] = lx(i, objnum, probability);
      }
    }
  ...
  private double lx(int k, int n, double p) {
    try {
      ...
      double probility = c * Math.pow(p, k) * Math.pow(1 - p, n - k);
      double lx = -(Math.log(probility) / Math.log((double) 2));
      return lx * probility;
    } catch (Exception e) {
      return 0;
    }
  ...
  /**.
   * 计算任意两个物体之间的最短逻辑距离。这里的逻辑距离是指：e1 和 e2 之 间通过最 少多少条边（relation）即可连接在一起。两个物体之间若无关系，
   * 则距离无穷大。 当所要计算的物体不再系统内时，返回-1
   * 
   * @param c  待计算的轨道系统
   * @param e1 待计算的轨道物体
   * @param e2 待计算的轨道物体
  ...
  public int getLogicalDistance(CircularOrbit c, E e1, E e2) {
    if (e1 == null || e2 == null) {
      throw new NullPointerException("getLogicalDistance argument is null pointer");
    }
    ...
    if (!vertex.get(0).getClass().getName().equals(e1.getClass().getName())) {
      throw new IllegalArgumentException(
          "getLogicalDistance object format does not match system, but was "
          + vertex.get(0).getClass().getName() + ", " + e1.getClass().getName());
    }
    E base = e1;
    E target = e2;
    // 检查是否有顶点未加入顶点集
  ...
  public double getPhysicalDistance(CircularOrbit c, E e1, E e2) {
    if (e1 == null || e2 == null) {
      throw new NullPointerException("getPhysicalDistance argument is null pointer");
    }
    List<E> vertex = new ArrayList<E>();
    @SuppressWarnings("unchecked")
    Iterator<E> t = c.iterator();
    ...
    if (c.getAngle(base) == -1 || c.getAngle(target) == -1) {
      return -1; // no angle
    }
    if (base.equals(target)) {
      return 0; // compare itself
    }
  ...
  /**.
   * 计算同端点两边另一个端点连线的长度，即两条有限长度的边从一个共同端点放射。输入两边长度和夹角度数 {@code a, b, angle} 。 其中
   * {@code angle} 为角度制，两条边长为正数。
   * 
   * @param a     三角形一条边长，大于0
  ...
  private double cosineFormula(double a, double b, double angle) {
    if (a <= 0 || b <= 0) {
      throw new IllegalArgumentException(
          "The length of the triangle is not positive, a " + a + ",b " + b);
    }
  ...
  /**.
   * 计算两个多轨道系统之间的差异。包括：轨道数的差异、具有相 同次序的轨道上物体数量的差异和物体的差 异（如果物体不需要区分，则不给出物体的差异，只给出数量
  ...
  public Difference getDifference(CircularOrbit c1, CircularOrbit c2) {
    if (c1 == null || c2 == null) {
      throw new NullPointerException("Null pointer in c1 or c2");
    }
    if ((c1 instanceof TrackGame) && !(c2 instanceof TrackGame)) {
      throw new IllegalArgumentException("Different types of systems");
    }
    if ((c1 instanceof AtomStructure) && !(c2 instanceof AtomStructure)) {
      throw new IllegalArgumentException("Different types of systems");
    }
    if ((c1 instanceof PersonalAppEcosystem) && !(c2 instanceof PersonalAppEcosystem)) {
      throw new IllegalArgumentException("Different types of systems");
    }
    ...
    /*
     * Get objects difference Use Set intersection operation, difference operation
     */
    if (di) {
    ...
    logger.info(
        "Get the Difference :" + c1.getClass().getSimpleName() 
