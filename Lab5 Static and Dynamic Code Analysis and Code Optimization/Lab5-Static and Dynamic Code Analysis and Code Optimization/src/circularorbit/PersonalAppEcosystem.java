...
/**.
 * 这是一个{@code mutable} 类型，表示一个 {@code 个人App生态系统}。用户日常使用手机上的 各个 App，根据各个 App
 * 的使用时间长度和频度，可以计算出每个 App 与该用户的“亲密度”。根据亲密度的不同，可以构造以该 用户为中心的 多级轨道结构，称之为“个人 App
 * 生态系统”：围绕用户有多条轨道，与 用户亲密度最高的若干 App 在最内层轨道，越往外的轨道上的 App 与用 户的亲密度越低。
 * 轨道系统固定5条轨道，从里向外亲密度逐渐 变低。App的亲密度的计算方法为：取当前轨道系统中的
 *
 */
public class PersonalAppEcosystem extends ConcreteCircularOrbit<Person, App> implements Cloneable {
   ...
   * 
   *    如果某个应用在某个时间段内被安装或被卸 载，那么它应该出现在该时间段的轨道 系统中；如果某个应用在某个时间段之前已被卸载，则不应出现在该时间段的轨道
   *    系统中。
   * 
  ...
  private void checkRep() {
    if (getCentralPoint() != null) {
      assert getCentralPoint().getName().matches(Regex.REGEX_LABEL) : 
          "Person name " + getCentralPoint().getName()
          + " is not label format";
    }
    ...
    while (it.hasNext()) {
      ...
      assert installing.get(a.getName()).overlap(duration) : "App " + a.getName()
          + " is uninstalling during this timespan";
    }
   ...
   * 
   * @param appName app的名称，app在系统中应存在,名称非空
  ...
  public String toString() {
    StringBuilder sb = new StringBuilder(
        "[PersonalAppEcosystem:" + getName() + ", Duration:" + duration.toString()
