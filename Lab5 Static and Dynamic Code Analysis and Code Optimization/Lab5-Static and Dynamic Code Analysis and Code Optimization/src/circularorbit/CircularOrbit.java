...
/**.
 * {@code CircularOrbit}是一个由唯一一个中心点和多条轨道构成的系统，在中心点 {@code L} 和 轨道上承载多个物体
 * {@code E}， 物体在轨道上可保持静止或持续 运动，物体与物体之间可存在联系。 将该类系统放在直角坐标系中， 可看作是以原点(0,0)
 * 为中心的多个同心圆.
 * 
 * <p>系统中的轨道都有自己的编号，由内向外的编号从0开始逐渐递增，且每条轨道的半径/权重/编号都不相同
 * 
 * <p>这个轨道系统是可变的，在运行过程中，轨道系统会根据不同的要求而发生变化。因此轨道系统是 {@code mutable}的。
 */
public interface CircularOrbit<L, E> extends Iterable<E> {
  ...
  /**.
   * 向系统添加中心物体，如果没有中心点物体，则添加中心点物体，返回null。若存在中心点物体， 则将中心点物体替换为新的中心物体，返回旧的物体。
   * 
   * @param centralPoint 需要添加的中心物体，中心物体非空
   * @return 如果系统之前没有中心物体则返回null，否则如果是新旧物体的替换则返回之前的那个 旧的中心物体。
   */
   ...
   * 不要求物体必须不同（例如电子都是相同的），添加的物体在track中没有次序，只是按照先后的存放 顺序存储。
   * 
  ...
  /**.
   * 向指定轨道添加物体，设置初始角度，物体必须为PhysicalObject类型物体或其子类，track在orbit中必须存在， angle为角度制，角度 0
   * <= angle < 360。不要求物体必须不同（例如电子都是相同的），添加 的物体在track中没有次序。
   * 
   * @param object 需要添加的轨道物体，物体必须为PhysicalObject类型物体或其子类
   * @param index  需要将物体添加到的轨道的编号，轨道编号 >= 0
   ...
   * 返回被移除track的对象。如果系统中的track是顺序的，那么该track顺位tracks依次前移一位。 即如果轨道顺序为
   * a-b-c-d，移去c，顺序变为 a-b-d
   * 
   ...
   * 特别的，如果系统中所有物体都一样（比如说电子），那么此方法会从最内层移除一个物体，如果物体包含角度， 将从初始角度最小的开始移除。
   * 
   ...
   * 特别的，如果轨道中所有物体都一样（比如说电子），那么此方法会随机移除，如果物体包含角度， 将从初始角度最小的移除。
   * 
  ...
  /**.
   * 添加中心物体和轨道物体的无向关系，关系只能添加一次且不能重复添加，中心点和轨道物体都必须 在轨道系统中存在。
   * 
  ...
  /**.
   * 移除中心物体和轨道物体关系，如果关系存在则移除，返回被移除关系的轨道物体对象，否则返回null。 要求该轨道系统中中心物体存在。中心点自身不能添加关系 。
   * 
   * @param orbitObject 需要解除关系的轨道物体
   * @return 如果关系存在则返回true。否则，如果该系统中不存在该轨道物体，或者 该轨道物体与中心物体没有关系，则返回false
   */
  boolean removeCentralRelation(E orbitObject);

  /**.
   * 添加轨道物体无向关系，两个轨道物体必须存在与轨道系统中。不要求所在轨道必须相同。
   * 
   * @param object1 需要添加关系的第一个物体
   * @param object2 需要添加关系的第二个物体
   * @return 添加关系成功返回true，否则返回false
   */
   ...
   * @return 如果两个物体的关系存在则移除，返回true，否则，如果两个物体不存在关系 ，返回false
   */
   ...
   * 
   * @return 所有与中心物体存在关系的轨道物体的集合，如果没有任何物体与中心点存在关系，则 返回空集
   */
  Set<E> centrals();
   ...
   * @return 返回所有与指定轨道物体有关系的轨道物体的集合，如果指定轨道物体不存在于系统或 没有物体与指定物体有关系则返回空集
   */
  Set<E> relatives(E object);

  /**.
   * 根据文件图输入和对应的解析器将轨道系统对象初始化，要求轨道系统对象为空图，否则之前的轨道系统
   * 对象不会保存，在初始化时会被破坏原有结构。例如，跑步比赛的轨道系统初始化需要对应的文件输入和 对应的跑步比赛格式的解析器。
   * 
   ...
   * 新的轨道必须在轨道系统中存在，轨道物体也必须存在。特别的，如果所有物体都一样，则从制定轨道上随机选择一个物体 跃迁。
   * 
   * @param object   需要修改的轨道物体对象
   ...
   * @return true如果指定轨道中指定物体存在，否则，如果指定轨道不存在或者指定物体不存在或者 轨道中不包含轨道物体，则返回false。
   */
  ...
  /**.
   * 获得指定物体的初始角度，特别的，如果系统中所有物体都一样（比如电子），那么将会随机选取一个物体 的初始角度返回，如果物体不存在初始角度，则返回-1。
   * 
   * @param object 指定的物体
  ...
  /**.
   * 对轨道物体按照编号/半径进行升序排序，返回排序后轨道索引与半径/编号的映射关系。 排序后轨道顺序将发生变化，因此之前不同轨道对应的索引可能变化。
   * 
   * @return
   */
