...
public class AppFactory extends PhysicalObjectFactory {
   ...
   * 
   * <p>其中， App 名称和公司为 label 类型， 版本是类似于使用诸如“.”、“_”、 “-”分割的 label，例如 v3.2-3_001、2.30
   * 等。功能描述和业务领域均带引 号，类 型是 sentence 类型
   * 
  ...
  public PhysicalObject create(ParamPackage pp) {
    ...
    if (!(pp instanceof AppParamPackage)) {
      throw new IllegalArgumentException("ParamPackage is not AppParamPackage");
    }

    AppParamPackage app = (AppParamPackage) pp;
    return new App(
        app.getName(), app.getCompany(), app.getVersion(), app.getDescripition(), app.getArea());
  }
