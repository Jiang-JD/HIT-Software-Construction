...
public class RaceViewController {
  ...
  void handleAddAthlete(ActionEvent event) {
    if (checkAddAthlete()) {
      Athlete a = (Athlete) af.create(
          "Athlete ::= <" + name.getText() + "," + number.getText() + "," + nation.getText()
          + "," + age.getText() + "," + score.getText() + ">");
      adda.add(a);
  ...
  private boolean checkRemoveTrack() {
    ...
    if (tmp.getTrackNum() <= Integer.parseInt(removetrack.getText()) 
        || Integer.parseInt(removetrack.getText()) < 0) {
  ...
  public void refreshVis() {
    ...
    vispane.getChildren().add(BuildVisualPane.drawCircularOrbit(
        tmp, vispane.getPrefWidth(), vispane.getPrefHeight()));
  ...
  /**
   * .Set Track game controller 
   * @param tgc track game controller
   */
