...
public class Timespan {
  ...
  /**.
   * 判断当前时间段是否与其他时间段相交，相交的定义为时间段有重合部分，特别的， 如果时间段之间仅仅有时间点重合，则判定为不相交，例如，t1的终止时间点与t2的
   * 起始时间点重合，t1与t2不相交。
   * 
   * @param ts 待判断时间段
  ...
  public boolean overlap(Timespan ts) {
    ...
    if (ts.getEnd().isBefore(start)) {
      return false;
    }
    if (ts.getStart().isAfter(end)) {
      return false;
    }

    if ((start.isBefore(ts.getStart()) 
        || start.equals(ts.getStart())) && end.isAfter(ts.getStart())) {
      return true;
    }

    if (((end.isAfter(ts.getEnd()) 
        || end.equals(ts.getEnd())) && start.isBefore(ts.getEnd()))) {
      return true;
    }
    if (ts.getStart().isBefore(start) && ts.getEnd().isAfter(end)) {
      return true;
    }
