...
/**.
 * 管理所有电子跃迁备忘录的类，管理所有备忘录状态， 负责存入，取出相关状态的备忘录
 *
 */
  ...
  public String toTable() {
    ...
    for (Memento m : memento) {
      ...
      for (int i = 0; i < m.getTracks().size(); i++) {
        ms.append(m.getTracks().get(i).getNumber() 
            + "/" + m.getMap().get(m.getTracks().get(i)).size() + ";");
      }
      sb.append("|" + index + "\t|" + m.getTime().format(dtf) 
          + "\t|" + m.getNucleus().getName() + "\t\t|"
          + m.getTracks().size() + "\t|" + ms.toString()
          + "\n+--------------------------------------------------------------------+\n");
      index++;
    }
    return sb.toString();
