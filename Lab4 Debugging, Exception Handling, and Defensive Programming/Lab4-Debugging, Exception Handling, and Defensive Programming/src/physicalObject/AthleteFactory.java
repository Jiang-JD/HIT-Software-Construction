...
public class AthleteFactory extends PhysicalObjectFactory {
	...
	public PhysicalObject create(ParamPackage pp) {
		...
		if(!(pp instanceof AthleteParamPackage)) throw new IllegalArgumentException("ParamPackage is not AthleteParamPackage");
		
		AthleteParamPackage app = (AthleteParamPackage)pp;
		return new Athlete(app.getName(), app.getNumber(),app.getNation(), app.getAge(), app.getPersonalBest());
	}
