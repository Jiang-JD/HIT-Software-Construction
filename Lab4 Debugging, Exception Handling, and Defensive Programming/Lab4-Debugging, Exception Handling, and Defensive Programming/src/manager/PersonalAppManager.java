 ...
 * 管理类中保管着所有App集合，App之间的关系，用户使用日志，App安装时间，用户名称，
 * 划分时间，管理器能够对这些内容进行操作管理。
 */
public class PersonalAppManager implements Cloneable {
	...
	/*
	 * Abstract Function:
	 * 	AF(apps, appsInstall, usageLog, relation, user, period) = 负责对用户软件生态系统中的App和日志进行维护，管理和划分的管理器
	 * 
	 * Rep Invariant:
	 * 	apps		不含null
	...
	/**
	 * 初始化管理器，将文本文件解析为一组App对象，App安装时间，使用日志，App合作关系，划分时间和用户。
	 * 输入文件所在路径，对管理器进行初始化.
	 * 划分时间默认为Day
	 * 
	 ...
	 */
	public boolean initial(String filePath) throws UndefinedElementException, IllegalElementFormatException, NoSuchElementException, LackOfComponentException, ElementLabelDuplicationException, IncorrectElementDependencyException {
		clear();
		PersonalAppEcosystemParser aep = new PersonalAppEcosystemParser();
		aep.parserFile(filePath);
		String fileText = aep.getText(filePath);
	...
	/**
	 * 由文本输入生成一组用户生态系统，生态系统根据不同的时间范围进行划分，
	 * 在调用此方法前，确保已经调用{@code initial} 方法对管理器进行初始化
	 * 
	 * @return 一组用户生态系统
	 */
	public List<PersonalAppEcosystem> generateEcosystems() {
		/* Check arguments*/
		if(apps.isEmpty()) throw new IllegalArgumentException("Apps not initialized");
		if(appsInstall.isEmpty()) throw new IllegalArgumentException("AppsInstall not initialized");
		if(usageLog.isEmpty()) throw new IllegalArgumentException("UsageLog not initialized");
		if(relation.isEmpty()) throw new IllegalArgumentException("Relation not initialized");
		if(user.equals("")) throw new IllegalArgumentException("User not initialized");
		...
		/* Generating intimacy map and build Ecosystem 
	...
	/**
	 * 一个计数器对象，用于统计次数
	 * https://www.jianshu.com/p/c42da3ad8743
	 */
	 ...
	 * 
	 * <p>{@code Hour} : 	<br>Find the nearest integer hour in the past, i.e. 2019/5/2 9:51:22 becomes 2019/5/2 9:00:00.
	 * 			Then make timespan in hours. 2019/5/2 9:00:00...2019/5/2 10:00:00...2019/5/2 11:00:00.
	 * 
	 * <p>{@code Day} : 	<br>Find the day of that date, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/5/2 00:00:00
	 * 			Then make timespan in days. 2019/5/2 00:00:00...2019/5/3 00:00:00...2019/5/4 00:00:00.
	 * 
	 * <p>{@code Week} : 	<br>Find the Monday of the week as the starting point, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/4/29 00:00:00
	 * 			Then make timespan in Weeks. 2019/4/29 00:00:00...2019/5/6 00:00:00
	 * 
	 * <p>{@code Month} : 	<br>Find the 1st of the month as the starting point, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/5/1 00:00:00
	 * 			Then make timespan in Weeks. 2019/5/1 00:00:00...2019/6/1 00:00:00
	 * 
	 * @return 一组时间划分
	 */
	private List<Timespan> timePartition() {
		...
		for(UsageLog l : usageLog) {
			if(l.getTime().isBefore(min)) min = l.getTime();
			if(l.getTime().isAfter(max)) max = l.getTime();
		}
		 ...
		 * 
		 * Hour: 	Find the nearest integer hour in the past, i.e. 2019/5/2 9:51:22 becomes 2019/5/2 9:00:00.
		 * 			Then make timespan in hours. 2019/5/2 9:00:00...2019/5/2 10:00:00...2019/5/2 11:00:00.
		 * 
		 * Day: 	Find the day of that date, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/5/2 00:00:00
		 * 			Then make timespan in days. 2019/5/2 00:00:00...2019/5/3 00:00:00...2019/5/4 00:00:00.
		 * 
		 * Week: 	Find the Monday of the week as the starting point, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/4/29 00:00:00
		 * 			Then make timespan in Weeks. 2019/4/29 00:00:00...2019/5/6 00:00:00
		 * 
		 * Month: 	Find the 1st of the month as the starting point, start in 00:00:00.i.e. 2019/5/2 9:51:22 becomes 2019/5/1 00:00:00
		 * 			Then make timespan in Weeks. 2019/5/1 00:00:00...2019/6/1 00:00:00
		 * 
		 */
		...
		switch(period) {
		case Hour: {
			start = earliest.minusMinutes(earliest.getMinute()).minusSeconds(earliest.getSecond()); // Set start time by hour
		...
		case Day: {
			start = earliest.minusMinutes(earliest.getMinute()).minusHours(earliest.getHour()).minusSeconds(earliest.getSecond());
			while(start.isBefore(latest)) {
		...
		case Week: {
			start = earliest.minusDays(earliest.getDayOfWeek().getValue()-1).minusMinutes(earliest.getMinute())
					.minusHours(earliest.getHour()).minusSeconds(earliest.getSecond());
			while(start.isBefore(latest)) {
		...
		case Month: {
			start = earliest.minusMinutes(earliest.getMinute()).minusHours(earliest.getHour()).minusSeconds(earliest.getSecond()).minusDays(earliest.getDayOfMonth()-1);
			while(start.isBefore(latest)) {
	...
	/**
	 * 根据时间段的划分将使用日志划分，某个时间段对应的使用日志规则为：<p>
	 * 如果使用日志对应的时间点在时间段内，那么这条使用日志应当属于这个划分，不考虑使用时长
	 * 
	...
	/**
	 * 根据时间段的划分将App划分，某个时间段对应的App规则为：<p>
	 * 如果某个应用在某个时间段内被安装或被卸 载，那么它应该出现在该时间段对应的划分中；
	 * <br>如果某个应用在某个时 间段之前已被卸载，则不应出现在该时间段的划分中；<br>如果某个应 
	 * 用在某个时间点被安装且后续没有被卸载，那么它就应始终出现在后续 各个时间段的划分中
	 * 
	 * @param timePartition 时间段划分
	 * @return 一组App的划分
	 */
	 ...
	 * @return
	 */
	private List<PersonalAppEcosystem> massProduction(List<Timespan> timePartition, List<List<UsageLog>> logPartition, List<List<String>> appPartition) {
		if(logPartition.size() != appPartition.size() || timePartition.size() != appPartition.size() || logPartition.size() != timePartition.size())
		...
		for(int i = 0 ; i < logPartition.size(); i++) {
			...
			for(String app: appPartition.get(i)) {
				...
				if(fre == null) {
					amap.put(app, 0.0);
					if(maxIntimacy < 0.0) maxIntimacy = 0.0;
					if(minIntimacy > 0.0) minIntimacy = 0.0;
				}
				else {
					int f = fre.get();
					int l = len.get();
					...
					amap.put(app, in);
					if(maxIntimacy < in) maxIntimacy = in;
					if(minIntimacy > in) minIntimacy = in;
				}
			}
			...
			for(double j = maxIntimacy; Math.abs(j - minIntimacy) >= 1e-10; j -= interval) {
				List<App> alist = new ArrayList<App>();
				
				for(Map.Entry<String, Double> e : amap.entrySet()) {
					if(Math.abs(j - interval - minIntimacy) <= 1e-10) {
						if((((j-e.getValue() <= 1e-10 && j-e.getValue() >= 0)|| j-e.getValue() > 1e-10) 
								&& (e.getValue() - (j - interval) >= 1e-10) || (e.getValue() - (j - interval) <= 1e-10 && (e.getValue() - (j - interval) >= 0)))) {
							alist.add(getApp(e.getKey()));
						}

					}
					else {
					if((((j-e.getValue() <= 1e-10 && j-e.getValue() >= 0)|| j-e.getValue() > 1e-10) && e.getValue() - (j - interval) >= 1e-10)) {
						alist.add(getApp(e.getKey()));
					}
					}
				}
			...
			/* Build Ecosystem */
			PersonalAppEcosystem pae = buildEcosystem("AppEco"+i, timePartition.get(i), trackmap, appPartition.get(i), user);
			ecoList.add(pae);
			intimacy.add(amap);
		}
		return ecoList;
	 ...
	 * @return 一个用户生态系统
	 */
	private PersonalAppEcosystem buildEcosystem(String name, Timespan ts, List<List<App>> trackmap, List<String> apps, Person user) {
		assert trackmap.size() == 5 : "buildEco the length of trackmap is less than 5, but "+ trackmap.size();
		/* Build Ecosystem */
		...
		for(int i = 0; i < 5; i++) {
			...
			for(int k = 0; k < trackmap.get(i).size(); k++) {
				...
				logger.info(pae.getName()+" add new App: "+trackmap.get(i).get(k).getName()+" on track "+(i+1));
			}
		}
		//add relation
		for(String app : apps) {
	...
	public App getApp(String name) {
		assert name != null : "App name is null pointer";
	...
	/**
	 * 计算两个 App 之间的逻辑距离，逻辑距离是指能够通过合作关系连接的最小边数，若没有连接，则逻辑距离为无穷大，
	 * 自身与自身的距离为0
	 * 
	 * @param a1 待计算的App
	 * @param a2 待计算的App
	...
	public int getDistance(App a1, App a2) {
		if(a1 == null || a2 == null) throw new IllegalArgumentException("getDistance argument is null pointer");
	...
	public Difference getDifference(Timespan t1, Timespan t2) {
		if(t1 == null || t2 == null) throw new IllegalArgumentException("Null pointer in time span");
		List<Timespan> timePartition  = new ArrayList<Timespan>();
		timePartition.add(t1);
		timePartition.add(t2);
		List<PersonalAppEcosystem> li = massProduction(timePartition, logPartition(timePartition), appPartition(timePartition));
	...
	public boolean add(App apps, AppInstallTime install) {
		if(apps == null) throw new NullPointerException("null pointer apps");
		if(install == null) throw new NullPointerException("null pointer install");
		if(getApp(apps.getName()) != null) return false;
	...
	public boolean addAll(List<App> apps, List<AppInstallTime> install) {
		if(apps == null) throw new NullPointerException("null pointer apps");
		if(install == null) throw new NullPointerException("null pointer install");
		if(apps.size()!= install.size()) throw new IllegalArgumentException("Not euqals in apps and install time");
		for(App a : apps) {
