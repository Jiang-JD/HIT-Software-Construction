...
public class FindMedianSortedArrays {
	...
	public double findMedianSortedArrays(int[] A, int[] B) {
		...
		/*
		 * 1. 将m+n修改为m+n+1
		 * why: i表示A中分界下标，j表示B中分界下标，两个数组的中位数条件为
		 * 		左边的数字和右边的数字个数应该相等，不考虑中间。所以为
		 * 		i+j = m-i + n-j + 1
		 * 		i+j = (m+n+1) / 2
		 * 		j = hafLen - i
		 */ 
		...
		while (iMin <= iMax) {
			/*
			 * 2. 将imin+imax+1 修改为 imin+imax
			 * why: i表示下标，下标的计算不需要+1
			 */
			...
			if (i < iMax && B[j - 1] > A[i]) {
				...
				/*
				 * 3. 将m+n+1 修改为 (m+n) % 2 != 0 (SpotBug提示) 
				 * why: 这里是判断A和B合并的数组数字个数是否为奇数，若为奇数，则maxLeft就
				 * 		表示中间的数字，直接返回即可。这里m和n表示AB数量，不应该+1
				 */
				if ((m + n) % 2 != 0) {
					return maxLeft;
				}
