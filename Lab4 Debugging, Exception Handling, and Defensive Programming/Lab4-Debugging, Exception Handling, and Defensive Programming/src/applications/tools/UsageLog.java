...
public class UsageLog {
	...
	public boolean within(Timespan ts) {
		if(ts == null) throw new NullPointerException("Timespan is null");
		
		if((ts.getStart().isBefore(time) || ts.getStart().equals(time))
				&& ts.getEnd().isAfter(time) || ts.getEnd().equals(time)) return true;
		return false;
	}
	
	@Override public String toString() {
        return "[UsageLog AppName:" + this.getName()
                + ", DateTime:" + this.getTime() +", Duration:"+ this.getDuration() 
                + "]";
    }
