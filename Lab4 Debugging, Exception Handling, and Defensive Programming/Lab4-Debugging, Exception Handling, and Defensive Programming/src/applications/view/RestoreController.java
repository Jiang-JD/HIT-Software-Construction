...
public class RestoreController {
	...
	private String s = "Click to choose the memento to restore";
    ...
    private void initialize() {
    	...
    	detailColumn.setCellValueFactory(cellDate -> {
    		...
    		for(int i = 0; i < m.getTracks().size(); i++) {
				sb.append("[Track:"+m.getTracks().get(i).getNumber()+"/"+m.getMap().get(m.getTracks().get(i)).size()+"]");
			}
    		return new ReadOnlyStringWrapper(sb.toString()); 
    	});
    	mementos.getSelectionModel().selectedItemProperty().addListener(
	            (observable, oldValue, newValue) -> updateMemento(newValue));
    }
