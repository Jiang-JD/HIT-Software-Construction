 ...
 *
 */
public class Application_CommandLine {
				...
				if (op.equals("y")) {
					...
					try {
						if (!tgm.initial(s)) {
							...
							| ElementLabelDuplicationException | TrackNumberOutOfRangeException e) {
						e.printStackTrace();
						show(Menu.ERRORFILE);
						break;
					}
				...
				if (op.equals("y")) {
					...
					try {
						if ((as = asp.initial(s)) != null) { //spotbug 标记 不修
							...
							| IncorrectElementLabelOrderException | IncorrectElementDependencyException
							| LackOfComponentException | ElementLabelDuplicationException e) {
						e.printStackTrace();
						show(Menu.ERRORFILE);
						break;
					}
		...
		while (f) {
			...
			case "5" : {
				...
				String regex = "((\\d{4}-(0[1-9]|1[0-2])-(0\\d|1\\d|2\\d|3[0-1]))"
						+ ",((0\\d|1\\d|2[0-3]):([0-5]\\d):([0-5]\\d)))=((\\d{4}-(0[1-9]|1[0-2])-(0\\d|1\\d|2\\d|3[0-1]))"
			...
			case "6": {
				...
				if((a = pam.getApp(s1)) != null && (b = pam.getApp(s2)) != null) {
					...
					if(dis == Integer.MAX_VALUE) showl("无穷大，之间没有联系");
					else showl(Integer.toString(dis));
					break;
				}
				else {
					showl("查找的App不存在");
